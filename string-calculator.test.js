...
test('throws an error with negative number', () => {
  expect(() => add('//;\n-1;1')).toThrow('Negatives not allowed. Number(s) causing error: -1')
})

test('throws an error with two negative numbers', () => {
  expect(() => add('//;\n-1;-2')).toThrow('Negatives not allowed. Number(s) causing error: -1, -2')
})
...
// Bonus 4
test('add 1, 2, 3, 4 support multiple delimiter of arbitrary length', () => {
  expect(add('//$,***,@\n1***2$3@4')).toBe(10)
})
